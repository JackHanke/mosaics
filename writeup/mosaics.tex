...
It is critical that we are able to compute the number  $\left|f^{-1}(\ell)\right|$

\begin{definition}
    $g: \mathbb{F}^{(m,n)} \to \mathbb{M}^{(m,n)}$ takes a framed binary lattice and constructs a mosaic by replacing all $\begin{smallmatrix} 0 & 0 \\ 0 & 0 \end{smallmatrix}$ and $\begin{smallmatrix} 1 & 1 \\ 1 & 1 \end{smallmatrix}$ cells with $T_{0}$, and replacing all other cells with the unique tile that maps to that cell under $f$. 
\end{definition}

Notice that $g$ only constructs polygon mosacis, and not messy polygon mosaics.

From the definition of $f$, mosaics that do not contain a polygon map to the binary lattice of all $\begin{smallmatrix} 0 & 0 \\ 0 & 0 \end{smallmatrix}$ cells.
...
\begin{definition}
    Let $\ell^*$ be the framed binary lattice with all $\begin{smallmatrix} 0 & 0 \\ 0 & 0 \end{smallmatrix}$ cells.
    \label{def:all zeros}
\end{definition}
...
It suffices to compute $|f^{-1}(\{\ell\})|$ for a given binary lattice $\ell$. 

\begin{definition}
    Let $v(\ell)$ for a binary lattice $\ell$ to be the product over all cells in $\ell$ with $$\begin{cases}
...
If we let $\ell$ be the binary lattice on the right of Figure \ref{fig:example of f mapping}, we have $v(\ell) = -7^{11}$.
\end{exmp}

\begin{definition}
    Define $q(\ell)$ to be the number of $01$ strings that appear in the top row of vertices of $\ell$.
\end{definition}

\begin{definition}
    Define $p(\ell)$ to be the number of polygons in. TODO
\end{definition}

\begin{prop} 
Let $\ell$ be a binary lattice such that the left-most and right-most column of vertices are labeled $0$ and $v(\ell) \neq 0$. Then we have 
$$\text{sign}(v(\ell)) = (-1)^{p(\ell)-q(\ell)}.$$
\end{prop}
...
\caption{Case format}
\label{fig: bit flip case format}
\end{figure}
...
\end{center}
\caption{}
\label{fig: bit flip case 6}
\end{subfigure}
\caption{Bit Flip Cases}
\end{figure}

In Case \ref{fig: bit flip case 1} TODO
