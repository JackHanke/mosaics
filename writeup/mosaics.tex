...
Hong and Oh TODO CITE gave the following upper and lower bounds on the number of polygon $(n,m)$-mosaics $p_{n,m}$, namely

$$2^{n+m-3} \left(\frac{17}{10}\right)^{(n-2)(m-2)} \leq p_{n,m} \leq 2^{n+m-3} \left(\frac{31}{16}\right)^{(n-2)(m-2)}.$$

We give an exact expression for $p_{n,m}$. We also introduce and enumerate \textit{messy polygon mosaics}, a variant on the previously studied polygon mosaics.
