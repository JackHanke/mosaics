...
It would be convenient to compute $f^{-1}(\ell)$ for any binary lattice $\ell$ ``cell-by-cell''. Let $u: \mathbb{L}^{(m,n)} \to \mathbb{Z}$  map a binary lattice $\ell$ to the product over all cells in $\ell$ with each term being $$\begin{cases}
...
where these terms come from the number of tiles that can map to a specific cell under $f$. However, as $|f^{-1}(\begin{smallmatrix} 0 & 0 \\ 0 & 0 \end{smallmatrix})| = 7$, for the $(m,n)$ binary lattice $\ell^*$, we would get
...
We have $u(\ell_1) = 7^4, u(\ell_2) = 7^4,$ and $u(\ell_3) = 1$. Note that any tile in $\mathbb{T}$ can map to cell $\begin{smallmatrix} 0 & 0 \\ 0 & 0 \end{smallmatrix}$. Additionally, $f(\text{Mosaic } 3) = \ell_3$. Each cell in the bottom two rows of $\ell_1$ map to $7$ possible tiles. However, $1$ of the $7^4$ permutations forms a new polygon in these bottom two rows. This is also true for the top two rows of $\ell_2$. Therefore, $u(\ell_1)$, $u(\ell_2)$, and $u(\ell_3)$ all count Mosaic $3$.
...
\begin{definition}
For a binary lattice $\ell$, let $q(\ell)$ to be the number of $01$ strings that appear in the top row of vertices.
\end{definition}
