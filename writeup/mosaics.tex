...
Proposition \ref{prop: u counts atleast g l} shows that the function $u(\ell)$ counts the mosaics with at least the polygons in $g(\ell)$. Consequently, a binary lattice $\ell$ counts $\mathcal{M}$ if the polygons in $g(\ell)$ are a subset of the polygons in $\mathcal{M}$. Therefore, $\mathcal{M}$ is counted $\sum_{p=0}^{P(\ell)}\binom{P(\ell)}{p}$ times, grouped by the size of the subset of polygons $p$. This then gives

$$\sum_{\ell \in \mathbb{F}^{(m,n)}}u(\ell) = \sum_{\mathcal{M} \in \mathbb{M}^{(m,n)}}\sum_{p=0}^{P(f(\mathcal{M}))}\binom{P(f(\mathcal{M}))}{p} = \sum_{\ell \in \mathbb{F}^{(m,n)}}\left(\sum_{p=0}^{P(\ell)}\binom{P(\ell)}{p}\right)\left|f^{-1}(\ell)\right|,$$

where the second equality comes from grouping terms by the set of mosaics that map to the same binary lattice. Following the same logic along with by Proposition \ref{prop: sign of v} gives

$$\sum_{\ell \in \mathbb{F}^{(m,n)}} v(\ell) = \sum_{\mathcal{M} \in \mathbb{M}^{(m,n)}}\sum_{p=0}^{P(f(\mathcal{M}))}(-1)^{p}\binom{P(f(\mathcal{M}))}{p} = \sum_{\ell \in \mathbb{F}^{(m,n)}}\left(\sum_{p=0}^{P(\ell)}(-1)^p\binom{P(\ell)}{p}\right)\left|f^{-1}(\ell)\right|,$$

where the second equality also comes from grouping terms by the set of mosaics that map to the same binary lattice. Finally, by the binomial theorem, for $P(\ell) > 0$ we have

$$\sum_{p=0}^{P(\ell)}(-1)^{p}\binom{P(\ell)}{p} = 0,$$

and as the only binary lattice with $P(\ell) = 0$ is $\ell^*$, we have

$$\sum_{\ell \in \mathbb{F}^{(m,n)}} v(\ell) = \binom{0}{0}|f^{-1}(\ell^*)| = |\mathbb{S}^{(m,n)}|.$$
...
\begin{proof}
TODO Substituting the values for $v$ completes the proof.
\end{proof}
