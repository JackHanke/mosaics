...
\begin{equation}
7^{mn} - \Big|\mathbb{P}^{(m,n)}\Big| = \Big|\bigcup_{i=1}^N A_i \Big|.
\label{eq: inc exl setup}
\end{equation}

\begin{prop}
Let $\mathcal{M}$ be a mosaic and let $S$ be the indices of the polygons $\mathcal{M}$. We then have
...
\textbf{TODO} immediate extension to polygon mosaics
